...
from fuxictr.pytorch.layers import EmbeddingLayer, FiGAT_Layer, FM_Layer, InnerProductLayer, LR_Layer
import pdb


class FiGAT_FM2(BaseModel):
    def __init__(self,
                 feature_map,
                 model_id="FiGAT_FM",
                 gpu=-1,
                 ...
                 **kwargs):
        super(FiGAT_FM2, self).__init__(feature_map,
                                    model_id=model_id,
                                    gpu=gpu,
        ...
        self.figat = FiGAT_Layer(self.num_fields,
                                 self.embedding_dim,
    ...
    def forward(self, inputs): # 引入了deepfm的思想，加上shallow部分，效果不好
        X, y = self.inputs_to_device(inputs)
        feature_emb = self.embedding_layer(X)

        lr_out = self.lr_layer(X)
        fm1_out = self.inner_product_layer(feature_emb)
        # lr_out = self.batch_norm(lr_out)
        # fm1_out = self.batch_norm(fm1_out)
        shallow_pred = lr_out + fm1_out
        # shallow_pred = self.batch_norm(shallow_pred)

        h_out = self.figat(feature_emb)
        # h_out = h_out.long()  # torch.tensor(h_out, dtype=torch.int64)
        # h_out = self.fc(h_out)  # h_out
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        fm2_out = self.inner_product_layer(h_out)
        # fm2_out = self.batch_norm(fm2_out)
        y_pred = fm2_out  # lr_out + fm2_out
