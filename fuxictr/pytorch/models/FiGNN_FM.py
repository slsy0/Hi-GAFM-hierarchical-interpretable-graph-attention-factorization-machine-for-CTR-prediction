...
from fuxictr.pytorch.layers import EmbeddingLayer, FiGNN_Layer, InnerProductLayer, LR_Layer


class FiGNN_FM(BaseModel):
    def __init__(self, 
                 feature_map, 
                 model_id="FiGNN", 
                 gpu=-1, 
                 ...
                 **kwargs):
        super(FiGNN_FM, self).__init__(feature_map,
                                    model_id=model_id, 
                                    gpu=gpu, 
        ...
        self.inner_product_layer = InnerProductLayer(feature_map.num_fields,
                                                     output="elementwise_product").cuda()  # product_sum_pooling
                    
    def forward(self, inputs):
        X, y = self.inputs_to_device(inputs)  # X: b x f   y: b x 1
        feature_emb = self.embedding_layer(X)
        h_out = self.fignn(feature_emb)  # b x f x emb_dim
        lr_out = self.lr_layer(X)
        fm_out = self.inner_product_layer(h_out)
        # y_pred = lr_out + fm_out
        y_pred = fm_out
        y_pred = self.fc(y_pred)  # batch_size x 1
...
class AttentionalPrediction(nn.Module):
    ...
    def forward(self, h):
        ...
        logit = (weight * score).sum(dim=1).unsqueeze(-1)  # b x 1  # tensor.sum(dim):tensor对dim维度进行求和，压缩维度
        return logit
