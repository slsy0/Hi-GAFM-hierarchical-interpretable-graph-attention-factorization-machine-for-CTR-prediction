...
from fuxictr.pytorch.layers import EmbeddingLayer, FiGAT_Layer2, FM_Layer, InnerProductLayer, LR_Layer, MLP_Layer
from fuxictr.pytorch.layers import BilinearInteractionLayer
import pdb


class AFM_MLP(BaseModel):
    def __init__(self,
                 feature_map,
                 model_id="FiGAT_FM",
                 gpu=-1,
                 ...
                 batch_norm=False,
                 **kwargs):
        super(AFM_MLP, self).__init__(feature_map,
                                   model_id=model_id,
                                   gpu=gpu,
        ...
        self.att_w = FiGAT_Layer2(self.num_fields,
                                  self.embedding_dim,
    ...
    def forward(self, inputs):
        ...
        copy_embedding = attention_sum  # torch.masked_select(h_out, self.diag_mask.unsqueeze(-1))
        inner_product_vec = self.inner_product_layer(attention_sum)   # interaction features
        dnn_input = torch.cat([copy_embedding.flatten(start_dim=1), inner_product_vec], dim=1)  # bs x 1561
