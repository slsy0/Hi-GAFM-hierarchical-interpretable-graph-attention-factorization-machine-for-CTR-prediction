...
class ONNv2(BaseModel):
    def __init__(self, 
        ...
        self.dnn = MLP_Layer(input_dim=embedding_dim * self.num_fields + self.interact_units,
                             ...
                             use_bias=True)
        self.embedding_layer = EmbeddingLayer(feature_map, embedding_dim * self.num_fields) # b x f x emb_dim*f
    ...
    def forward(self, inputs):
        ...
        field_wise_embedding = self.embedding_layer(X).view(-1, self.num_fields, self.num_fields, self.embedding_dim)  # bs x f x f x emb_dim
        copy_embedding = torch.masked_select(field_wise_embedding, self.diag_mask.unsqueeze(-1)).view(self.batch_size, -1) # embedding features # bs x f*emb_dim
        ffm_out = self.ffm_interaction(field_wise_embedding)  # interaction features  bs x f*(f-1)/2
