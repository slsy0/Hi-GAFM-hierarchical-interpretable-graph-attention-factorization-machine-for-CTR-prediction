...
from fuxictr.pytorch.layers import EmbeddingLayer, FiGAT_Layer, FM_Layer, InnerProductLayer, LR_Layer, MLP_Layer
import pdb


class FM_MLP(BaseModel):
    def __init__(self,
                 feature_map,
                 model_id="FiGAT_FM",
                 gpu=-1,
                 ...
                 batch_norm=False,
                 **kwargs):
        super(FM_MLP, self).__init__(feature_map,
                                    model_id=model_id,
                                    gpu=gpu,
    ...
    def forward(self, inputs):
        ...
        # gru
        a = h_out.view(-1, self.embedding_dim)
        ...
        copy_embedding = h_out   # torch.masked_select(h_out, self.diag_mask.unsqueeze(-1))  # embedding features
        inner_product_vec = self.inner_product_layer(h_out)  # interaction features
