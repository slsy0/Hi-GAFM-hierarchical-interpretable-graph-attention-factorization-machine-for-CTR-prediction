...
class GAFM2(BaseModel):
    def __init__(self,
                 ...
                 **kwargs):
        super(GAFM2, self).__init__(feature_map,
                                    model_id=model_id,
                                    gpu=gpu,
    ...
    def forward(self, inputs):
        ...
        for i in range(self.gat_layers):
            fm_out.append(self.inner_product_layer(attention_sum[i]))
            output.append(lr_out + fm_out[i])

        # fm_out = lr_out + fm_out

        # fm_z = torch.zeros([fm_out.size(0), fm_out.size(1), feature_emb.size(1)-fm_out.size(2)])
        # fm_z = fm_z.to(device)
        # fm_out = torch.cat([fm_out, fm_z], dim=2)

        # fm_out = fm_out.view(-1, att_w.size(1), att_w(2))
        # attention_sum = torch.sum(att_w * fm_out, dim=1)
        # att_out = self.dropout2(attention_sum)

        # 1 做平均
        out = output[0]
        for i in range(self.gat_layers-1):
            out = torch.add(out, output[i+1])
        y_pred = torch.div(out, self.gat_layers)

        '''
        # 2 注意力
        output = torch.stack(output, dim=1).cuda()
        output = self.batch_norm3(output)
        # output = self.dropout(output)

        # out = self.self_attention(output)
        # out = self.attentional_score(output)
        out = self.fc(output)
        y_pred = out
        '''
...
class AttentionalPrediction(nn.Module):
    ...
    def forward(self, h):   # 1000x3x1
        score = self.mlp1(h).squeeze(-1)  # b x f   numpy.squeeze()函数  axis=-1时删除倒数第一个维度   1000x3
