...
class FiGATv2_Layer2(nn.Module):
    def __init__(self,
                 num_fields,
                 embedding_dim,
                 gat_layers=3,
                 ...
                 use_residual=True,
                 device=None):
        super(FiGATv2_Layer2, self).__init__()
    ...
    def forward(self, feature_emb):
        ...
        for i in range(self.gat_layers):
            if self.reuse_graph_layer:
                ...
                att_w = self.att_w[i](h, g, feature_emb.shape[0], feature_emb.shape[2])  # a = self.gat[i](h, g)
...
class GraphAttentionLayer(nn.Module):
    ...
    def forward(self, inp, adj, batch_size, out_features):
        """
        inp: input_fea [bs, N, in_features]  in_features表示节点的输入特征向量元素个数
        adj: 图的邻接矩阵 维度[bs, N, N] 非零即一，数据结构基本知识
        """
        # 定义可训练参数，即论文中的W和a
