...
class GraphLayer(nn.Module):
    ...
    def forward(self, g, h):    # 如何提取权重？
        h = h.unsqueeze(-1)     # batch x fields x emb_dim x 1
        h_out = torch.matmul(self.W_out, h)  # broadcast multiply   h_out: batch x fields x emb_dim x 1
        h_out = h_out.squeeze(-1)   # batch x fields x emb_dim
        aggr = torch.bmm(g, h_out)  # 对存储在两个批batch1和batch2内的矩阵进行批矩阵乘操作  g: batch x fields x fields
        # aggr: batch x fields x emb_dim
