...
class FiGAT_Layer3(nn.Module):
    def __init__(self,
                 ...
                 use_residual=True,
                 device=None):
        super(FiGAT_Layer3, self).__init__()
        ...
        if reuse_graph_layer:
            self.att_w = GraphAttentionLayer(num_fields, embedding_dim)
        else:
            self.att_w = nn.ModuleList([GraphAttentionLayer(num_fields, embedding_dim)
                                      for _ in range(gat_layers)])
        self.gru = nn.GRUCell(num_fields, num_fields) if use_gru else None
        self.src_nodes, self.dst_nodes = zip(*list(product(range(num_fields), repeat=2)))   # source，destination
        # Python中的product()函数用于计算两个或多个可迭代对象的笛卡尔积。在这段代码中，product()函数用于为图中的源节点和目标节点生成所有可能的索引对。然后使用zip()函数将这些对解压缩到两个独立的源节点和目标节点列表中。
        self.leaky_relu = nn.LeakyReLU(negative_slope=0.01)
        self.W_attn = nn.Linear(embedding_dim * 2, 1, bias=False)  # nn.Linear()用于设置神经网络全连接层

    def build_graph(self, feature_emb):
        src_emb = feature_emb[:, self.src_nodes, :]  # source   [batch_size x field_num^2 x emb_dim]
        dst_emb = feature_emb[:, self.dst_nodes, :]  # destination  [batch_size x field_num^2 x emb_dim]
        concat_emb = torch.cat([src_emb, dst_emb], dim=-1)  # 对embedding层连接  [batch_size x field_num^2 x emb_dim*2]
        alpha = self.W_attn(concat_emb)  # fignn中用self.leaky_relu(self.W_attn(concat_emb))
        alpha = alpha.view(-1, self.num_fields, self.num_fields)
        mask = torch.eye(self.num_fields).to(self.device)
        alpha = alpha.masked_fill(mask.bool(), float('-inf'))  # mask.byte与mask.bool 将对角线元素全部变成负无穷
    ...
    def forward(self, feature_emb):
        ...
        for i in range(self.gat_layers):
            if self.reuse_graph_layer:
                att_w = self.att_w(h, g, feature_emb.shape[0], feature_emb.shape[2])  # a = self.gat(h, g)

            else:
                att_w.append(self.att_w[i](h, g, feature_emb.shape[0], feature_emb.shape[2]))  # a = self.gat[i](h, g)
            # if self.gru is not None:   # 门控循环单元 (Gated Recurrent Unit，GRU) 是一种常用的GRNN（门控循环神经网络）
            #     att_w = att_w.view(-1, self.embedding_dim)
...
class GraphAttentionLayer(nn.Module):
    ...
    def forward(self, inp, adj, batch_size, out_features):
        ...
        # [batch_size, N, N, 1] => [batch_size, N, N] 图注意力的相关系数（未归一化）
